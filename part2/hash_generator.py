...
def hash_image(path_or_url: str) -> dict[str, str]:
    ...
    if not path_or_url or not path_or_url.strip():
        return {"sha256": "", "status": "unavailable", "detail": "No image path/URL provided."}
    ...
    if not path_or_url.startswith(("http://", "https://")):
        ...
        return {"sha256": "", "status": "unavailable", "detail": f"Local file not found: {path_or_url}"}
    ...
    try:
        ...
        return {"sha256": "", "status": "unavailable", "detail": f"Could not download image: {exc}"}
...
def canonicalize(post_data: dict) -> str:
    ...
    otherwise the raw ``post_image`` value (normalised).
    """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: py part2/hash_generator.py <path_to_post_json>")
        sys.exit(1)
